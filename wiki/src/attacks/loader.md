...
Parses, validates, and exposes YAML attack scenarios as typed Python dataclasses.
Also reads raw Markdown document files referenced by scenarios.
...
| `AttackScenario` | dataclass | Full scenario: name, description, type, source_channel, steps, expected decisions, optional document text |
| `load(path)` | function | Parse and validate a single YAML scenario file; returns `AttackScenario` |
| `load_document(path)` | function | Read a raw `.md` file and return its text content |
| `load_all(directory)` | function | Load all `.yaml` files in a directory, skipping `schema.yaml`; returns list of `AttackScenario` |
...
- Future: `runner/interface.py` (I6) — scenario execution pipeline

## See also

- [[src/attacks/index]] — package overview
